...
void nothing_to(EnergyPlusData &state, void * see_here) {
    ...
    ShowContinueError(
        state,
        format(
            "It might be this: {} or that: {}", state.data->Node(1).Temp,state.data->CoolVector[x].attributeY
        )
    );
    ...
    void anotherFunction(EnergyPlusData &state) {
        if (true) {
            ...
            ShowContinueError(
                state,
                format(
                    "{} should be zero when the fuel type is electricity.", state.dataIPShortCut->cNumericFieldNames(10)
                )
            );
        ...
        ShowSevereError(
            state,
            format(
                "{}{}: invalid: {}", RoutineName, cCurrentModuleObject, state.dataIPShortCut->cAlphaFieldNames(2)
            )
        );
        ErrorsFound = true;
    }
